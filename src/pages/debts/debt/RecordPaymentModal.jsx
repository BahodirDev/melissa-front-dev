...
function RecordPaymentModal({
  ...
  const validate = () => {
    ...
    if (formData.amount > outstanding) {
      newErrors.amount = `Summa qarz qoldig'idan (${formatCurrency(
        outstanding
      )} so'm) oshmasligi kerak`;
    }
    setErrors(newErrors);
    ...
    try {
      ...
      if (response?.status === 200 || response?.status === 201) {
        ...
        const errorMsg =
          ...
          "To'lov qayd etishda xatolik";
        toast.error(errorMsg);
      }
    } catch (error) {
      setOptimisticUpdate(null);
      const errorMsg =
        error?.response?.data?.message || "To'lov qayd etishda xatolik";
      toast.error(errorMsg);
    } finally {
      setLoading(false);
    }
    ...
    <>
          ...
          <div className="record-payment-form">
            <div className="record-payment-field">
              ...
              {errors.amount && (
                <span
                  id="amount-error"
                  className="record-payment-error"
                  role="alert"
                >
                  {errors.amount}
                </span>
            ...
            <div className="record-payment-summary">
              ...
              <div className="record-payment-summary-row">
                <span>Qoladi:</span>
                <strong>
                  {formatCurrency(Math.max(0, outstanding - formData.amount))}{" "}
                  so'm
                </strong>
              </div>
            </div>
      ...
      <Confirmation
        ...
        body={
          <div>
            <p>
              <strong>{client?.name}</strong> uchun{" "}
              <strong>{formatCurrency(formData.amount)} so'm</strong> to'lov
              qayd etilmoqda.
            </p>
            <p>
              To'lov usuli:{" "}
